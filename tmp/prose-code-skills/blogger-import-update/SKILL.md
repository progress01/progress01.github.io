...
Edit an existing article in place and preserve unknown front-matter keys; do not rebuild it from the new-article example. Keep `date` and `permalink` by default; if the user explicitly requests a correction, apply it and check internal links. For formatting-only or image-conversion changes, keep any existing `updated`; for substantive content changes, set or add one `updated` using a reliable `Asia/Taipei` timestamp, without duplicate fields or moving it backward. If the user explicitly requests correction of an erroneous timestamp, follow that request and verify generated `dateModified >= datePublished`. If a hand-written `description` or `excerpt` exists, confirm it still represents the revised body and do not add either field just to satisfy this check. JSON-LD is generated by the theme; never paste it into article Markdown. After generation, check `headline`, canonical URL, `datePublished`, `dateModified`, and `image` against the article title, permalink, dates, and actual cover or representative image. See the [project Schema.org notes](<C:/Blog/blog-1988/Schema.org JSON-LD 實作手冊.md>) when needed.

## Verification checklist

- Inspect imported prose for accidental code blocks, especially lines with four leading spaces or a tab after blank lines or HTML boundaries. Compare generated `.post-body pre`, `pre code`, and `figure.highlight` contents against the intended article text; preserve genuine code examples. Repair only the source indentation or delimiters proven to cause the error, then confirm the same words render as ordinary paragraphs after a clean build. Do not globally trim indentation or rebuild an article from its feed for this formatting repair. Preserve wording, metadata, image markup, and existing `alt`; missing image alt is a separate issue and must not broaden a prose-as-code repair.

- For every article created or edited in this workflow, read [blog-excerpt-check](<C:/Users/111/.codex/skills/blog-excerpt-check/SKILL.md>) and run `node tools/excerpt-check.js --post "source/_posts/<path>.md"`; inspect the generated target card when `description`/`excerpt` or imported HTML affects presentation. Skip only index-only JSON/YAML or `life-index-only` updates; a pure image repair still checks the target and reports pre-existing overlong text as legacy without expanding the repair into prose edits.
