...
3. For a new article, create `source\_posts\歌曲推薦\歌曲推薦-XXX.md` using this exact structure, replacing the placeholders with the user's data. Keep the existing inline structure so older song posts and new posts are rendered consistently by the shared song style:
   ...
   Keep the attribution, `<br>`, and experience text flush left in the actual Markdown file. Four leading spaces or a tab outside an HTML block can turn prose into an indented code block; blank lines can end a raw HTML block and expose its remaining indented lines to Markdown parsing. Preserve the song card's HTML structure, and remove only indentation confirmed to cause incorrect rendering. Do not globally trim indentation: lists, genuine code examples, and HTML structure may need it.

   For an existing update, edit the current file in place and preserve unknown front-matter keys; do not rebuild it from this new-article example. Keep `date` and `permalink` by default; if the user explicitly requests a correction, apply it and check internal links. For formatting-only or image-conversion changes, keep any existing `updated`; for substantive content changes, set or add one `updated` using a reliable `Asia/Taipei` timestamp, without duplicate fields or moving it backward. If the user explicitly requests correction of an erroneous timestamp, follow that request and verify generated `dateModified >= datePublished`. If a hand-written `description` or `excerpt` exists, confirm it still represents the revised body and do not add either field just to satisfy this check. JSON-LD is generated by the theme; never paste it into the article. After generation, check `headline`, canonical URL, `datePublished`, `dateModified`, and `image` against the article title, permalink, dates, and the shared cover/body image.
...
- For a prose-as-code repair, compare the source with the generated target article's `.post-body pre`, `pre code`, and `figure.highlight` elements. Confirm the affected words appear as ordinary article text after a clean build, with no lost paragraphs or visible escaped `<br>`/HTML. Real code blocks remain valid; their mere presence is not a failure. Preserve image markup (including existing `alt`), metadata, and wording during this formatting-only repair; report missing image alt separately if found, without expanding the task.

- For every article created or edited in this workflow, read [blog-excerpt-check](<C:/Users/111/.codex/skills/blog-excerpt-check/SKILL.md>) and run `node tools/excerpt-check.js --post "source/_posts/<path>.md"`; inspect the generated song card when `description`/`excerpt` or the fixed HTML card affects presentation. Skip only index-only JSON/YAML or `life-index-only` updates; a pure image repair still checks the target and reports pre-existing overlong text as legacy without expanding the repair into prose edits.
...
- Confirm generated JSON-LD uses the article's headline, canonical URL, dates, and the same cover image when a cover is present.
