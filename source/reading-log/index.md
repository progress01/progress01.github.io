...
<style>
  ...
  .life-index-now,
  .life-index-entry-grid,
  .life-index-next-grid { display: grid; gap: 12px; margin-top: 14px; }
  .life-index-now { grid-template-columns: repeat(3, 1fr); }
  .life-index-now-card,
  .life-index-entry-card,
  .life-index-next-card {
    display: block;
    position: relative;
    padding: 17px;
  ...
  .life-index-entry-card:hover,
  .life-index-next-card:hover { color: var(--life-ink); transform: translate(-2px, -2px); box-shadow: 4px 4px 0 rgba(74, 48, 40, .2); }
  .life-index-now-card:nth-child(2) { background: #f2dfc1; }
  .life-index-now-card:nth-child(3) { background: #e7ddd0; }
  ...
  .life-index-entry-card p,
  .life-index-next-card p { margin: 0; color: #6b5547; font-size: 13px; line-height: 1.7; }
  ...
  .life-index-next-grid { grid-template-columns: minmax(0, 1.2fr) minmax(220px, .8fr); }
  .life-index-next-card { min-height: 170px; }
  .life-index-list-label { color: var(--life-orange); font-size: 10px; }
  .life-index-list { margin: 12px 0 0; padding: 0; list-style: none; }
  .life-index-list li { padding: 7px 0; border-bottom: 1px dashed var(--life-line); color: #5d4538; font-size: 13px; line-height: 1.5; }
  .life-index-list li::before { margin-right: 8px; color: var(--life-orange); content: "□"; font-family: "Courier New", monospace; }
  .life-index-note { display: flex; align-items: center; color: #6b5547; background: #efe1c9; font-size: 15px; line-height: 1.9; }
  .life-index-note strong { display: block; margin-bottom: 9px; color: var(--life-brown); }
  ...
  @media (max-width: 767px) {
    ...
    .life-index-hero, .life-index-next-grid { grid-template-columns: 1fr; }
...
<div id="life-index">
  ...
  <section class="life-index-section">
    ...
    <div id="life-index-current" class="life-index-now">
      <div class="life-index-next-card">正在讀取現在的內容……</div>
    </div>
  </section>
  ...
  <section class="life-index-section">
    <div class="life-index-section-heading"><span>C-01</span><h2>接下來</h2><small>NO DEADLINE</small></div>
    <div class="life-index-next-grid">
      <div class="life-index-next-card">
        <div class="life-index-list-label">NEXT ON THE DESK</div>
        <ul id="life-index-next" class="life-index-list">
          <li>正在讀取清單……</li>
        </ul>
      </div>
      <div id="life-index-next-note" class="life-index-next-card life-index-note"><div>正在讀取備註……</div></div>
    </div>
  </section>
...
<script>
  (function() {
    ...
    var nextContainer = document.getElementById('life-index-next');
    var nextNote = document.getElementById('life-index-next-note');
    var currentSlots = [
      { key: 'read', fallbackLabel: 'READ / 閱讀中' },
    ...
    function renderLifeIndex(data) {
      ...
      (data.next || []).forEach(function(item) {
        ...
        listItem.textContent = typeof item === 'string' ? item : item.title;
        nextContainer.appendChild(listItem);
      });
      if (!nextContainer.children.length) {
        nextContainer.innerHTML = '<li>目前沒有待整理的項目。</li>';
      }
      nextNote.innerHTML = '';
      var note = document.createElement('div');
      var noteTitle = document.createElement('strong');
      noteTitle.textContent = '這裡不追蹤進度。';
      note.appendChild(noteTitle);
      note.appendChild(document.createTextNode(data.nextNote || '想讀就放進來，讀完就寫成文章。'));
      nextNote.appendChild(note);
    }
    ...
    fetch('/life-index.json')
      ...
      .catch(function() {
        currentContainer.innerHTML = '<div class="life-index-next-card">目前的內容暫時無法讀取。</div>';
        nextContainer.innerHTML = '<li>清單暫時無法讀取。</li>';
        nextNote.innerHTML = '<div>請稍後再回來看看。</div>';
      });
  })();
